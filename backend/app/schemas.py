...
class GenerateMeshResponse(BaseModel):
    ...
    mesh_name: str = Field(
        description="Basename without extension; unique on server (stem + id suffix)."
    )
    source_stem: str = Field(
        description="Sanitized upload filename stem (no extension, no server id) for display / client assets."
    )
